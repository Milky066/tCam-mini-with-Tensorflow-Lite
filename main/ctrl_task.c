...
// Board/IO related
static int ctrl_brd_type; // Board type
static int ctrl_if_mode;  // Determine if we're using WiFi or Ethernet
...
void ctrl_task()
{
	...
	ctrl_task_init(); // If succesful, the default state is CRTL_ST_STARTUP
...
static void ctrl_handle_notifications()
{
	...
	if (xTaskNotifyWait(0x00, 0xFFFFFFFF, &notification_value, 0)) // Wait with no block time
	{
		if (Notification(notification_value, CTRL_NOTIFY_STARTUP_DONE)) // & notification_value with CTRL_NOTIFY_STARTUP_DONE
		{
