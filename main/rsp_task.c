...
void rsp_task()
{
	...
	while (1)
	{
		...
		handle_notifications(); // image_pending = true, stream_on = false for "get_image" command
...
static void handle_notifications()
{
	...
	if (xTaskNotifyWait(0x00, 0xFFFFFFFF, &notification_value, 0))
	{
		...
		// if (Notification(notification_value,))
		...
		if (Notification(notification_value, RSP_NOTIFY_CMD_PREDICT_IMG_MASK))
		{
