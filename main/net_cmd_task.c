...
void net_cmd_task()
{
	char rx_buffer[256]; // Maximusize of command 256 letters inclusive of headers
	char addr_str[16];
	int err;
	int flag;
	int len;
	...
	// This is the main loop for listening commands from TCP/IP socket.
	while (1)
	{
		...
		while (1)
		{
			...
			else if (len == 0)
			{
				...
				break;
				// Gets out of this while loop to line 190, closing the socket and restart the session
			}
			// Data received
			else
			{
